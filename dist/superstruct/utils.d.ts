 ...
 */
export type UnionToIntersection<Union> = (Union extends any ? (arg: Union) => any : never) extends (arg: infer Type) => void ? Type : never;
/**
 * Assign properties from one type to another, overwriting existing.
 */
export type Assign<Type, OtherType> = Simplify<OtherType & Omit<Type, keyof OtherType>>;
/**
 ...
 * unions as a match.
 */
export type IsMatch<Type, OtherType> = Type extends OtherType ? OtherType extends Type ? Type : never : never;
/**
 * Check if a type is an exact match.
 */
export type IsExactMatch<Type, OtherType> = (<Inner>() => Inner extends Type ? 1 : 2) extends <Inner>() => Inner extends OtherType ? 1 : 2 ? Type : never;
/**
 * Check if a type is a record type.
 */
export type IsRecord<Type> = Type extends object ? string extends keyof Type ? Type : never : never;
/**
 * Check if a type is a tuple.
 */
export type IsTuple<Type> = Type extends [any] ? Type : Type extends [any, any] ? Type : Type extends [any, any, any] ? Type : Type extends [any, any, any, any] ? Type : Type extends [any, any, any, any, any] ? Type : never;
/**
 * Check if a type is a union.
 */
export type IsUnion<Type, Union extends Type = Type> = (Type extends any ? (Union extends Type ? false : true) : false) extends false ? never : Type;
/**
 ...
 */
export type ObjectType<Schema extends ObjectSchema> = Simplify<Optionalize<{
    [K in keyof Schema]: Infer<Schema[K]>;
}>>;
/**
 * Omit properties from a type that extend from a specific type.
 */
export type OmitBy<Type, Value> = Omit<Type, {
    [Key in keyof Type]: Value extends Extract<Type[Key], Value> ? Key : never;
}[keyof Type]>;
/**
 * Normalize properties of a type that allow `undefined` to make them optional.
 */
export type Optionalize<Schema extends object> = OmitBy<Schema, undefined> & Partial<PickBy<Schema, undefined>>;
/**
 ...
 */
export type PickBy<Type, Value> = Pick<Type, {
    [Key in keyof Type]: Value extends Extract<Type[Key], Value> ? Key : never;
}[keyof Type]>;
/**
 * Simplifies a type definition to its most basic representation.
 */
export type Simplify<Type> = Type extends any[] | Date ? Type : // eslint-disable-next-line @typescript-eslint/ban-types
{
 ...
 * `tuple` structs.
 */
export type InferStructTuple<Tuple extends AnyStruct[], Length extends number = Tuple['length']> = Length extends Length ? number extends Length ? Tuple : InferTuple<Tuple, Length, []> : never;
type InferTuple<Tuple extends AnyStruct[], Length extends number, Accumulated extends unknown[], Index extends number = Accumulated['length']> = Index extends Length ? Accumulated : InferTuple<Tuple, Length, [...Accumulated, Infer<Tuple[Index]>]>;
export {};
